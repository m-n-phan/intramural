...
export function Schedule() {
  ...
  const editForm = useForm<Partial<InsertGame>>({
  ...
  const getEligibleTeams = (selectedSportId: number | undefined, selectedGender: string | undefined, selectedDivision?: string) => {
    if (!teams || !Array.isArray(teams) || !selectedSportId || !selectedGender) return [];
  ...
  const getEligibleAwayTeams = (selectedSportId: number | undefined, selectedGender: string | undefined, homeTeamId: number | undefined, selectedDivision?: string) => {
    const eligibleTeams = getEligibleTeams(selectedSportId, selectedGender, selectedDivision);
    return eligibleTeams.filter((team: Team) => team.id !== homeTeamId);
  };
  ...
  const selectedHomeTeam = Array.isArray(teams) ? teams.find((team) => team.id === watchedHomeTeamId) : null;
  const selectedDivision = selectedHomeTeam?.division;
  
  // Get edit home team to determine division for filtering
  const selectedEditHomeTeam = Array.isArray(teams) ? teams.find((team) => team.id === watchedEditHomeTeamId) : null;
  ...
  useEffect(() => {
    if (watchedEditHomeTeamId && editForm.getValues("awayTeamId")) {
      const awayTeam = Array.isArray(teams) ? teams.find((team) => team.id === editForm.getValues("awayTeamId")) : null;
  ...
  const onSubmit = (data: InsertGame) => {
    ...
    createGameMutation.mutate(submitData as InsertGame);
  };
  ...
  const onEditSubmit = (data: Partial<InsertGame>) => {
    ...
    updateGameMutation.mutate({ id: selectedGame.id, updates: submitData as Partial<InsertGame> });
  };
  ...
  const getFilteredGames = (games: Game[]) => {
    if (!games) return [];
  ...
  const groupGamesByDate = (games: Game[] | undefined) => {
    if (!games) return {};
    const filteredGames = getFilteredGames(games);
    const grouped = filteredGames.reduce((acc: { [key: string]: Game[] }, game) => {
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <Select onValueChange={(value) => field.onChange(Number(value))} value={field.value?.toString() ?? ""}>
                          ...
                          <SelectContent>
                            {getEligibleTeams(watchedSportId, watchedGender).map((team) => (
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <Select onValueChange={(value) => field.onChange(Number(value))} value={field.value?.toString() ?? ""}>
                          ...
                          <SelectContent>
                            {getEligibleAwayTeams(watchedSportId, watchedGender, watchedHomeTeamId, selectedDivision ?? undefined).length > 0 ? (
                              getEligibleAwayTeams(watchedSportId, watchedGender, watchedHomeTeamId, selectedDivision ?? undefined).map((team) => (
                                <SelectItem key={team.id} value={team.id.toString()}>
                                  {team.name} ({team.division})
                                </SelectItem>
                              ))
                        ...
                        <FormMessage />
                        {getEligibleAwayTeams(watchedSportId, watchedGender, watchedHomeTeamId, selectedDivision ?? undefined).length === 0 && watchedHomeTeamId > 0 && (
                          <p className="text-sm text-muted-foreground">
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <Select onValueChange={(value) => field.onChange(Number(value))} value={field.value?.toString() ?? ""}>
                          ...
                          <SelectContent>
                            {getEligibleTeams(watchedEditSportId, watchedEditGender).map((team) => (
                              <SelectItem key={team.id} value={team.id.toString()}>
                                {team.name} ({team.division})
                              </SelectItem>
                            ))}
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <Select onValueChange={(value) => field.onChange(Number(value))} value={field.value?.toString() ?? ""}>
                          ...
                          <SelectContent>
                            {getEligibleAwayTeams(watchedEditSportId, watchedEditGender, watchedEditHomeTeamId, selectedEditDivision ?? undefined).length > 0 ? (
                              getEligibleAwayTeams(watchedEditSportId, watchedEditGender, watchedEditHomeTeamId, selectedEditDivision ?? undefined).map((team) => (
                                <SelectItem key={team.id} value={team.id.toString()}>
                                  {team.name} ({team.division})
                                </SelectItem>
                              ))
                        ...
                        <FormMessage />
                        {getEligibleAwayTeams(watchedEditSportId, watchedEditGender, watchedEditHomeTeamId, selectedEditDivision ?? undefined).length === 0 && watchedEditHomeTeamId && watchedEditHomeTeamId > 0 && (
                          <p className="text-sm text-muted-foreground">
                            Only teams from the same division and gender category can play against each other.
                          </p>
                        )}
                          ...
                          <div className="flex items-center space-x-3">
                            ...
                            <div>
                              <div className="font-medium text-foreground">
                                {(Array.isArray(teams) && teams.find((t) => t.id === game.homeTeamId)?.name) || 'Home Team'} vs {(Array.isArray(teams) && teams.find((t) => t.id === game.awayTeamId)?.name) || 'Away Team'}
                              </div>
                              <div className="text-sm text-muted-foreground flex items-center space-x-2">
                                <span>{game.venue ? game.venue : ''}</span>
